    ...
    if (operation === "update") {
        ...
        if (errors) {
            return {
                video: null,
                message:
                    "Error: Something went wrong while updating video in database!",
        ...
        if (errors) {
            return {
                video: null,
                message:
                    "Error: Something went wrong while inserting video in database",
                errors: JSON.stringify(errors),
            };
        }
...
export async function getWatchItAgainVideosByUser(userData, token) {
    const variables = {
        userEmail: userData.email,
    };
    const gqlWatchItAgainQuery = `
    query getWatchItAgainVideos ($userEmail: String!) {
      user_videos(where: {user_email: {_eq: $userEmail}, watched: {_eq: true}}) {
        id
        user_email
        video_id
        watched
  ...
  `;
    ...
    const { data } = await fetchGraphQL(
        gqlWatchItAgainQuery,
        "getWatchItAgainVideos",
        variables,
        token
    );
    const output = data.user_videos;

    return output;
}

export async function getLikedVideos(userData, token) {
    const variables = {
        userEmail: userData.email,
    };
    const gqlLikedVideosQuery = `
    query getLikedVideos ($userEmail: String!) {
      user_videos(where: {user_email: {_eq: $userEmail}, liked: {_eq: 1}}) {
        id
        user_email
        video_id
        watched

...
export async function insertOrUpdateDbVideo(
    ...
    return operation === "update"
        ? await fetchGraphQL(gqlUpdateVideo, "setUserVideo", variables, token)
        : operation === "insert"
        ? await fetchGraphQL(gqlInsertVideo, "insertVideo", variables, token)
        : { message: "Error. Invalid operation.", data: null };
}
...
export async function checkDbUserVideo(userData, videoId, token) {
    try {
        ...
        if (errors) {
            ...
            const { data: newVideo, errors } = await insertOrUpdateDbVideo(
                "insert",
                videoConstructor,
                token
            );
            ...
            video = newVideo?.insert_user_videos_one;
        }

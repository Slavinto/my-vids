...
export const getVideos = async (query) => {
    const SEARCH_URL = `search?part=snippet&q=${query}&maxResults=25`;
...
const getOutput = (data) => {
    if (data?.error) {
        console.log("Youtube API error.", data.error.errors[0].reason);
        return [];
    }

    const output = data.map((videoObj, idx) => {
        const id = videoObj?.id?.videoId ? videoObj.id.videoId : videoObj.id;
